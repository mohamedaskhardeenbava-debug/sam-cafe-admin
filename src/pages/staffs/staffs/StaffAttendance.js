...
/*
  ...
  - Today → no record needed; the render logic shows a checkbox for
    today purely from `isToday`, without requiring a saved entry, so
    nothing is written to the server just to make today render
...
export default function StaffAttendance({ adminData, setAdminData }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const staff of adminData.staff) {
        ...
        try {
          // PATCH, not PUT: this only touches `attendance`, so it must
          // not resend the rest of the staff record. A full PUT here
          // was re-including idProof/bonafide/training untouched —
          // which re-runs the server's file-upload validation on every
          // attendance save, and fails with 400 for any staff record
          // whose stored file fields aren't in the exact validator-
          // expected data-URL shape (e.g. legacy records). PATCH's
          // partial body leaves those fields out entirely, so they're
          // never re-validated for a write that never touched them.
          const res = await api.patch(`/staff/${staff.id}`, { attendance: updatedAtt });
  ...
  // ── Seed today's entry as "unmarked" — removed ─────────────────
  // This used to PUT an { status: "unmarked" } entry (a status
  // outside the documented present/absent/leave enum) for every
  // staff member on every load, which the server rejected with 400,
  // and which could also race with the auto-absent effect above
  // writing to the same staff record at the same time. It's
  // unnecessary: the render logic below already shows today's
  // checkbox whenever `isToday` is true, with no saved record
  // required.
  ...
  const saveAttendance = async (staffId, date, data) => {
    ...
    const updated = idx >= 0
      ...
      : [...existing, { date, ...data }];
    try {
      // PATCH, not PUT — see the auto-absent effect above for why:
      // only `attendance` needs to change here, and a full PUT would
      // resend idProof/bonafide/training untouched, re-triggering the
      // server's file validation on a write that has nothing to do
      // with those fields.
      const res = await api.patch(`/staff/${staffId}`, { attendance: updated });
